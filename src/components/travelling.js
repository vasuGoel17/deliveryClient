...
const Travelling = () => {
  ...
  const userget = async () => {
    ...
    const res = await fetch("/api/getusert", {
      method: "GET",
      headers: {
  ...
  const handleClick = async () => {
    ...
    const data = await fetch("/api/searcht", {
      method: "post",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    console.log("naya res: ", res);
    console.log("nayaaa res: ", res.show);
    if (res.status === 200 || !data) {
      navigate("/searcht", { state: { show: res.show } });
    } else {
  ...
  return (
    <div style={{ position: "relative", minHeight: "100vh", width: "100%" }}>
      ...
      <div className="container shadow-container min-height-con mt-5 overflow-hidden round-edge">
        ...
        <div className="p-2">
          <Tablet userdatat={userdatat}></Tablet>
        </div>
      </div>
